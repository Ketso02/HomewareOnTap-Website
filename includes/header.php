        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link <?php echo (basename($_SERVER['PHP_SELF']) == 'about.php') ? 'active' : ''; ?>" href="<?php echo SITE_URL; ?>/pages/static/about.php">About Us</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link <?php echo (basename($_SERVER['PHP_SELF']) == 'contact.php') ? 'active' : ''; ?>" href="<?php echo SITE_URL; ?>/pages/static/contact.php">Contact</a>
                    </li>
                </ul>
                ...
                <div class="d-flex align-items-center">
                    <a href="<?php echo SITE_URL; ?>/pages/cart.php" class="cart-icon" id="cartIcon">
                        ...
                        <span class="cart-badge cart-count-badge">
                            <?php
                            $badge_count = 0;
                            if (isset($pdo)) {
                                $sess_id = session_id();
                                ...
                                if ($uid) {
                                    $s = $pdo->prepare("SELECT SUM(ci.quantity) as total FROM cart_items ci JOIN carts c ON ci.cart_id = c.id WHERE c.user_id = ?");
                                    $s->execute([$uid]);
                                } else {
                                    $s = $pdo->prepare("SELECT SUM(ci.quantity) as total FROM cart_items ci JOIN carts c ON ci.cart_id = c.id WHERE c.session_id = ?");
                                    $s->execute([$sess_id]);
                                }
                                $badge_count = (int)($s->fetch(PDO::FETCH_ASSOC)['total'] ?? 0);
                                }
                            echo $badge_count;
                            ?>
                        </span>
