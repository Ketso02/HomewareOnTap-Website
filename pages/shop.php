...
<body>
                        ...
                        <div class="col-md-6 col-xl-4 mb-4">
                            <div class="product-card">
                                <div class="product-image position-relative">
                                    <?php
                                    // Supports both old manually saved images and new uploaded images
                                    if (!empty($product['image'])) {
                                        if (strpos($product['image'], 'assets/uploads/products/') === 0) {
                                            // New uploaded image path saved from admin
                                            $imageUrl = SITE_URL . '/' . $product['image'];
                                        } else {
                                            // Old/manual image filename saved in database
                                            $imageUrl = SITE_URL . '/assets/img/products/primary/' . $product['image'];
                                        }
                                    } else {
                                        // Fallback image
                                        $imageUrl = SITE_URL . '/assets/img/products/primary/default-product.jpg';
                                    }
                                    ?>

                                    <a href="product-detail.php?id=<?php echo $product['id']; ?>">
                                        <img class="lazy-image"
                                             data-src="<?php echo htmlspecialchars($imageUrl); ?>"
    ...
    <script>
        // Constants
        const CART_CONTROLLER_URL = '../system/controllers/CartController.php';
    ...
    $('#cartToast').fadeOut();
                        ...
                        result.items.forEach(function(item) {
                            cartHtml += `
                                <div class="cart-item d-flex align-items-center mb-3 pb-3 border-bottom">
                                    <img src="${item.image && item.image.startsWith('assets/uploads/products/') 
                                            ? '<?php echo SITE_URL; ?>/' + item.image 
                                            : '<?php echo SITE_URL; ?>/assets/img/products/primary/' + (item.image || 'default-product.jpg')}"
                                         alt="${item.name}"
                                         class="rounded"
                ...
                error: function() {
                    $('#cart-items-container').html(`
                        <div class="text-center py-4 text-danger">
                            ...
                            <p>Could not load cart. Please refresh the page.</p>
                        </div>
                    `);
                    console.error('Network error loading cart items');
                }
            });
        }
        ...
        function removeFromCart(cartItemId) {
            $.ajax({
                ...
                error: function() {
                    showToast('Network error removing item from cart', 'error');
                }
            });
        }
    </script>
